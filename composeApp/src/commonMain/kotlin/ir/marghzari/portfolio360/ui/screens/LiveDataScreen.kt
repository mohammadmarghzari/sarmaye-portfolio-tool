...
fun LiveDataScreen(appState: AppState) {
    ...
    var fgState by remember { mutableStateOf<UiState<FearGreedData>>(UiState.Loading) }

    val prices = appState.prices
    var newsTicker by remember(prices) { mutableStateOf(prices?.tickers?.firstOrNull().orEmpty()) }
    var newsState by remember { mutableStateOf<UiState<List<NewsItem>>>(UiState.Empty) }
    var seasTicker by remember(prices) { mutableStateOf(prices?.tickers?.firstOrNull().orEmpty()) }

    suspend fun loadFearGreed() {
        fgState = UiState.Loading
        val fetched = appState.fearGreed.fetch()
        fgState = if (fetched != null) UiState.Success(fetched) else UiState.Error("دریافت Fear & Greed ناموفق بود.")
    }

    fun loadNews() {
        scope.launch {
            newsState = UiState.Loading
            val items = appState.news.fetch(newsTicker)
            newsState = if (items.isEmpty()) UiState.Empty else UiState.Success(items)
        }
    }

    LaunchedEffect(Unit) { loadFearGreed() }
    ...
    LazyColumn(verticalArrangement = Arrangement.spacedBy(16.dp), modifier = Modifier.fillMaxWidth()) {
        ...
        item {
            StateHost(
                state = fgState,
                onRetry = { scope.launch { loadFearGreed() } },
        ...
        if (prices == null) {
            item {
                EmptyState(
                    title = "هنوز داده‌ای دریافت نشده",
                    hint = "ابتدا از تب «تخصیص دارایی» داده پرتفوی را دانلود کنید تا اخبار و Seasonality نمادها فعال شود.",
                )
            }
        } else {
            item {
                SimpleDropdown("نماد", newsTicker, prices.tickers, { it }, { t -> newsTicker = t; loadNews() })
            }
            item {
                AppButton(
                    text = "دریافت اخبار $newsTicker",
                    onClick = { loadNews() },
                    loading = newsState is UiState.Loading,
                )
            }
            item {
                StateHost(
                    state = newsState,
                    onRetry = { loadNews() },
                    emptyTitle = "خبری برای نمایش نیست",
                    emptyHint = "دکمه «دریافت اخبار» را بزنید یا نماد دیگری انتخاب کنید.",
